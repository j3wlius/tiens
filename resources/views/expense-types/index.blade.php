                                ...
                                @foreach($expenseTypes as $type)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('expense-types.edit', $type) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                            <form action="{{ route('expense-types.toggle', $type) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <form action="{{ route('expense-types.destroy', $type) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this expense type?')">Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
